...
## Summary of Findings 
Six machine learning models were built to help doctors identify at-risk patients in order to provide prevention methods needed to help their patients be proactive with managing their health. Based on the six models produced, it can be noted that the model that outperformed all other models was determined to be the Neural Network model. The six models can be ranked as follows from greatest predictability to least predictability: Neural Network, Logistic Regression, Random Forest, Bernoulli Naive Bayes, Linear Discriminant Analysis, and Decision Tree. It should be noted that all models performed with great proficiency based on their precision, recall, F1, accuracy, and ROC scores, and therefore are sufficient models that can be further expanded on and employed. The results serve as a source of hope that the problem of identifying at-risk heart disease patients in a timely manner can be mitigated through machine learning efforts. Overall, this can lead to the outcome of doctors being able to visit and assist more patients efficiently. Giving patients the opportunity to be diagnosed quickly can lead to them receiving preventative care or treatments which can benefit their overall health. These findings bridge the gap in healthcare settings with patient satisfaction. This is the case in terms of diagnoses considering that the time doctors spend looking through the medical history of a patient to make an accurate diagnosis cuts down on the time a patient has to share their personal concerns with the doctor. The results show potential in reducing this barrier through the automation of the diagnoses process which will lead to better doctor-patient interactions, overall patient satisfaction, and thus this can lead to better health outcomes.

## Business Question
How can we better predict heart disease to reduce the incidence of heart disease, improve doctor-patient interactions, and increase overall profitability of hospitals? 
